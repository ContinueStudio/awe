...
**问题**：用户反馈"主界面和编辑界面都没有顶部栏了"

**根因分析**：`App.tsx` 从 `37e29e9`（v0.3.8c 正常版本）到 HEAD 被大量删减，丢失了 370+ 行代码：
- `selectMode` 状态（框选模式）
- `showRunHistory` 状态（运行历史抽屉）
- `selectedNodeIds` 状态（画布多选节点）
- `closeConfirmOpen` 状态（关闭确认对话框）
- `clipboard` 状态（节点剪贴板复制/粘贴）
- `errorNodeIds` 状态（错误节点高亮）
- `CustomTitleBar` 组件（frameless 窗口标题栏）
- 大量编辑器功能函数

**根因**：在 `bf8ea0a` 提交中，我只用了一个简单的 App.tsx（不包含这些高级功能），覆盖了原本包含这些功能的版本。

**修复**：
- `App.tsx`：`git checkout 37e29e9 --` 恢复完整版本
- 重新添加 `TrashPage` 导入和 `trash` 路由
- 修复 `RunHistoryDrawer` 的 `onRun` 属性（因为组件接口在 HEAD 已变更）
- 更新版本号 `v0.3.6` → `v0.3.7`

**提交**：`47965bc` → push to main

**避坑**：修改任何已有大量功能的文件时，必须先查看当前文件内容，不能用一个简化版本直接覆盖。修改前先用 `git diff` 确认变更范围。
