...
## 1.9 v2.20 增量变更（相对 v2.19）
- **版本号**：PRD v2.20 / frontend v0.3.7a / 2026-07-06
- **背景**：v2.19 的「复选框列 + 顶部批量操作栏」反馈过于明显地占用了行高，且对单行操作形成视觉干扰。改用更优雅的键控多选交互。

### 1.9.1 交互重构
* **变更文件**：`src/pages/HomePage.tsx`
* **操作细节**：
  - **移除**：表头全选复选框列、每行的复选框、顶部"取消选择"和"删除选中(N)"红色按钮
  - **新增**：`handleRowClick(id, e)` 统一处理名称点击
    - **普通点击** → 选中并打开编辑器（保持原行为）
    - **Shift + 点击** → 范围选择（从 lastSelectedId 到当前 id 之间的所有行）
    - **Ctrl/Cmd + 点击** → 多选切换（toggle）
  - **新增键控交互**：
    - `Delete` / `Backspace` → 批量删除选中工作流（带 confirm 二次确认）
    - `ESC` → 清除所有选择
  - **新增状态**：`lastSelectedId` 记录锚点，支持 Shift 范围选择
  - **顶部提示**：选中状态下，副标题显示"Shift/Ctrl+点击名称多选 · Delete 批量删除 · ESC 取消"

### 1.9.2 视觉一致性
* 列表行选中态保持 `rgba(59,130,246,0.06)` 浅蓝高亮
* grid 列数从 5 列恢复为 4 列（与 v2.18 一致）
* 鼠标 hover 行为：选中行不再被 hover 覆盖

### 1.9.3 修复（v2.19 → v2.20 沉淀）
* **405 Method Not Allowed**：v2.19 引入 `POST /api/workflows/batch-delete` 时，如果后端没重启（运行旧版代码）会出现 405 报错。**必须重启后端**才能生效
* **后端启动路径**：`cd backend && ./venv/Scripts/python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 8765`
* **验证命令**：`curl -X POST http://127.0.0.1:8765/api/workflows/batch-delete -H "Content-Type: application/json" -d '{"ids":[]}'` → 期望 `{"ok":true,"deleted":0,"total":0}`

### 1.9.4 避坑要点（v2.20 沉淀）
* **Shift 范围选择需锚点**：单独 Shift+点击不切换，只锚定最后一次普通点击的位置
* **Delete 键冲突**：画布编辑器的 Delete 删除节点 vs 列表的 Delete 批量删除工作流，二者共存，靠当前焦点（focus target）区分；HomePage 监听到时检查 target 是否在 input/textarea
* **批量删除前 confirm**：必须二次确认，避免误删

---

## 1.10 v2.21 增量变更（相对 v2.20）
- **版本号**：PRD v2.21 / frontend v0.3.7b / 2026-07-06
- **背景**：v2.20 改为键控多选后，"普通点击=进入编辑" 和 "普通点击=选中" 产生冲突。
...
* **新交互**：
  - **单击名称** → 仅选中（不高亮操作按钮）
  - **双击名称** → 进入编辑器
  - **Shift + 点击** → 范围选择（保持 v2.20 行为）
  - **Ctrl/Cmd + 点击** → 多选切换（保持 v2.20 行为）
  - **Delete / Backspace** → 批量删除选中（保持 v2.20 行为）
  - **ESC** → 清除选择（保持 v2.20 行为）
* **设计理念**：单击=安全选择（不触发重操作），双击=明确意图（编辑器打开）

### 1.10.2 验证
* ✅ TypeScript 编译零错误
* ✅ Vite 生产构建：1592 modules / 244.82 kB / gzip 74.35 kB
* ✅ 新 hash：`index-DTpQusAy.js`

---

## 1.11 v2.22 增量变更（相对 v2.21）
- **版本号**：PRD v2.22 / frontend v0.3.7c / 2026-07-06
- **背景**：v2.20-v2.21 引入键控多选后，右键菜单的"删除"按钮仍调 `deleteOne(menu.id)`，导致多选状态下右键删除只删一条。

### 1.11.1 交互变更
* **变更文件**：`src/pages/HomePage.tsx`
* **新交互**：
  - 新增 `smartDelete(primaryId)`：右键删除时，**如果 primaryId 已在 selectedIds 中**（多选状态）→ 调用 `batchDelete()` 批量删除全部选中
  - 右键菜单文本动态化：
    - 单选状态：`删除`（红色 danger 项）
    - 多选状态：`删除选中 (N)`（红色 danger 项）
  - **fallback**：右键一个未选中的行 → 走 `deleteOne(primaryId)` 单删

### 1.11.2 行为矩阵
| 操作 | 删除范围 |
|------|---------|
| 单选 + 右键删除 | 删 1 个 |
| 多选 3 个 + 右键删除 | 删 3 个（批量） |
| 多选 3 个 + 右键第 4 行删除 | 删 1 个（primaryId 不在 selectedIds） |
| Delete 键 | 删 selectedIds 全部 |
| ESC | 不删除 |
...
## 1.14 v2.25 增量变更（相对 v2.24）
- **版本号**：PRD v2.25 / launcher v2.25 / 2026-07-06
- **背景**：`start.bat` 默认调 `python start.py`，没加 `--window`，仍然走"弹浏览器"分支。

### 1.14.1 变更
* **`start.bat`**：默认参数改为 `--window`，并保留 `%*` 让用户可加额外参数（如 `--dev` 启 Vite HMR）

### 1.14.2 启动命令矩阵
| 命令 | 行为 |
|------|------|
| `start.bat` | 后端 + 桌面窗口（默认） |
| `start.bat --dev` | 后端 + Vite dev + 桌面窗口（HMR 调试） |
